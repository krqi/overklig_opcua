...
class CGenerator:
    ...
    def print_datatype(self, datatype, namespaceMap):
        ...
        return "{\n" + \
               ...
               "    " + binaryEncodingId + ", \n" + \
               "    sizeof(UA_" + idName + "), \n" + \
               "    " + self.get_type_kind(datatype) + ", \n" + \
               "    " + pointerfree + ", \n" + \
               "    " + self.get_type_overlayable(datatype) + ", \n" + \
               "    " + str(len(datatype.members)) + ", \n" + \
               "    %s_members" % idName + "  \n" + \
               "}"
    ...
    def print_members(datatype, namespaceMap):
        ...
        for i, member in enumerate(datatype.members):
            ...
            m += " \n"
            m += ("    true" if member.is_array else "    false") + ", \n"
            m += ("    true" if member.is_optional else "    false") + "  \n}"
            if i != size:
                m += ","
            members += m
            before = member
...
#endif ''' % self.parser.outname.upper())
